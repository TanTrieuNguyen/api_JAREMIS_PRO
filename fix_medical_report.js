// Script to fix medical report template - removes emoji and improves layout
const fs = require('fs');
...
// Pattern to find and replace the entire medical report endpoint
const startPattern = "app.get('/api/patient-records/:patientId/medical-report'";
const startIndex = content.indexOf(startPattern);

if (startIndex === -1) {
  console.error('❌ Could not find medical-report endpoint');
  process.exit(1);
}

// Find the end of the endpoint (next app.get or app.post)
const endPattern = "\napp.";
const endIndex = content.indexOf(endPattern, startIndex + 100);

if (endIndex === -1) {
  console.error('❌ Could not find end of endpoint');
  process.exit(1);
}

console.log(`✅ Found endpoint from ${startIndex} to ${endIndex}`);
console.log(`   Length to replace: ${endIndex - startIndex} characters`);
...
const newEndpoint = `app.get('/api/patient-records/:patientId/medical-report', (req, res) => {
  try {
    ...
    patientRecords.forEach((record, index) => {
      ...
      visitSections += \`
        <div class="section">
          <h2 style="color: #333; font-size: 13pt; font-weight: bold; margin: 15px 0 10px 0; text-transform: uppercase; border-bottom: 2px solid #000; padding-bottom: 8px;">III. LÝ DO ĐẾN KHÁM BỆNH LẦN \${visitNum}</h2>
      ...
      notesSections += \`
        <div class="section notes-section">
          <h3 style="color: #333; font-weight: bold; text-transform: uppercase; margin: 10px 0 8px 0;">Chỉ định và dặn dò lần khám \${visitNum}</h3>
          <div contenteditable="true" class="placeholder-field" style="border: 1px solid #999; padding: 10px; min-height: 100px; border-radius: 4px; background: #fff;" data-placeholder="Ghi chú dặn dò của bác sĩ...">
            \${record.consultation || 'Không có dặn dò'}
          </div>
        </div>
      \`;
    });
      ...
      <body>
        ...
        <div class="container" id="medical-record">
          ...
          <div class="section">
            ...
            <div class="row">
              Họ và tên: <strong contenteditable="true" class="placeholder-field" data-placeholder="...">\${patientInfo.name || '.............................'}</strong>
            </div>
            <div class="row">
              Tuổi: <span contenteditable="true" class="placeholder-field" data-placeholder="...">\${patientInfo.age || '..'}</span>
              <span style="margin-left: 30px;">Giới tính: <label><input type="checkbox" \${patientInfo.gender === 'Nam' ? 'checked' : ''}> Nam</label> <label><input type="checkbox" \${patientInfo.gender === 'Nữ' ? 'checked' : ''}> Nữ</label></span>
            </div>
            <div class="row">
              Số CCCD/Hộ Chiếu: <span contenteditable="true" class="placeholder-field" data-placeholder="................................................................">................................................................</span>
            </div>
            <div class="row">
              Ngày cấp: <span contenteditable="true" class="placeholder-field" data-placeholder="...../.../.....">...../.../.....</span>
              <span style="margin-left: 30px;">Nơi cấp: <span contenteditable="true" class="placeholder-field" data-placeholder="................................">................................</span></span>
            </div>
            <div class="row">
              Địa chỉ: <span contenteditable="true" class="placeholder-field" data-placeholder="................................................................">................................................................</span>
            </div>
            <div class="row">
              Số thẻ BHYT: <span contenteditable="true" class="placeholder-field" data-placeholder="................................">................................</span>
              <span style="margin-left: 30px;">Giá trị đến: <span contenteditable="true" class="placeholder-field" data-placeholder="...../.../.....">...../.../.....</span></span>
            </div>
            <div class="row">
              Ngày khám: <span contenteditable="true" class="placeholder-field" data-placeholder="...../.../.....">\${new Date(latestRecord.timestamp).toLocaleDateString('vi-VN')}</span>
            </div>
          </div>
        ...
        <script>
          ...
          function exportDOCX() {
            ...
            const html = element.innerHTML
              .replace(/<div class="notes-section">.*?<\\/div>/gs, '');
...
console.log('✅ Medical report endpoint updated successfully!');
